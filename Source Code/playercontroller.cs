...
public class playercontroller : MonoBehaviour {
	...
	//Variable for gluttony level
	public static float burgersEaten;
	public float growthMultiplier = 1.1f; //MAKE SURE THIS VALUE MATCHES SUPERSIZE.CS AND PROJECTILECONTROLLER.CS
	...
	void FixedUpdate () 
	{
		...
		if (grounded && Input.GetAxis ("Jump") > 0 && releasedSpacebar) {
			...
			myAnim.SetBool ("isGrounded", grounded);
			rb.AddForce (new Vector2 (0, jumpHeight*Mathf.Pow(growthMultiplier,burgersEaten)), ForceMode2D.Impulse);
			Debug.Log ("Jumped!");
		}
		//Debug.Log (grounded);
		...
		if (Time.time > nextWallJump && releasedSpacebar) {
			...
			if (touchingWall && Input.GetAxis ("Jump") > 0) {
				touchingWall = false;
				releasedSpacebar = false;
				rb.AddForce (new Vector2 (Input.GetAxis ("Horizontal") * wallJumpDist* Mathf.Pow(growthMultiplier,burgersEaten), wallJumpDist/1.2f* Mathf.Pow(growthMultiplier,burgersEaten)), ForceMode2D.Impulse);
		...
		if (dash.dashState != DashState.Dashing) {
			myAnim.SetFloat ("speed", Mathf.Abs (move));
			rb.velocity = new Vector2 (move * maxSpeed * Mathf.Pow (growthMultiplier, burgersEaten / 2), rb.velocity.y);
		}
	...
	void fireBubble()
	{
		if (Time.time > nextFire) {
			...
			if (facingRight) {
				...
				//Modify bullet size for gluttony level
				bullet.transform.localScale = new Vector2 (bulletSize*Mathf.Pow(growthMultiplier,burgersEaten), bulletSize*Mathf.Pow(growthMultiplier,burgersEaten));
			}
		 	else if (!facingRight) {
